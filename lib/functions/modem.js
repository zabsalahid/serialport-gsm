...
module.exports = function(SerialPort) {
  ...
  modem.incomingSMSIndicationDelay = 500;
  modem.incomingSMSIndicationTimeoutsById = new Map();
  modem.logger = {
    debug: function() {}
  };
  ...
  modem.open = function(device, options, callback) {
    ...
    options.path = device
    modem.port = SerialPort(options, (error) => {
      if (error) {
        callback(error)
      } else {
        let result = {
    ...
    parts.forEach(part => {
      ...
      newparts.forEach(newpart => {
        ...
        if (modem.incomingSMSIndication && newpart.startsWith('+CMTI:')) { // New Message Indicator with SIM Card ID, After Receiving Read The Message From the SIM Card
          ...
          if (modem.incomingSMSIndicationTimeoutsById.has(id)) {
            clearTimeout(modem.incomingSMSIndicationTimeoutsById.get(id))
          }
          modem.incomingSMSIndicationTimeoutsById.set(id, setTimeout((id) => {
            modem.incomingSMSIndicationTimeoutsById.delete(id)
            modem.readSMSById(id)
