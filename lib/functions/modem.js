...
module.exports = function(SerialPort) {
    ...
    try {
      if (number && message) {
        ...
        for (let i = 0; i < parts.length; i++) {
          let item = modem.executeCommand([`AT+CMGS=${(parts[i].toString().length / 2) - 1}`,`${parts[i].toString()}\x1a`],function(data, error) {
            if (!data) {
          ...
          }, false, 30000, messageID, message, number, false)
  ...
  modem.executeCommand = (command, callback, priority, timeout, messageID, message, recipient, activateProcessing) => {
    ...
    if (Array.isArray(command)) {
      item.command = command.shift()
      item.additionalData = command
    } else {
    ...
    item.on('timeout', () => {
      modem.logger.debug(`Command Response timed out: ${item.command}`)
  ...
  modem.executeNext = () => {
    ...
    if (item.additionalData) {
      item.additionalData.forEach((data) => {
        modem.port.write(`${data}\r`)
        modem.logger.debug(`Modem Write Data: ${data}`)
      })
    }
  }
