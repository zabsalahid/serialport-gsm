    ...
    try {
      if (number && message) {
        ...
        if (alert) {
          ...
          parts = submit.getParts()
        } else {
          parts = smsPdu.generateSubmit(number, message);
        }
        ...
        for (let i = 0; i < parts.length; i++) {
          ...
          let message = ''
          if (alert) {
            length = (parts[i].toString().length / 2) - 1
            message = parts[i].toString()
          } else {
            length = parts[i].length
            message = parts[i].hex
          }

          modem.executeCommand(`AT+CMGS=${length}`, (data) => { }, false, 100)
          modem.executeCommand(`${message}` + '\x1a', function (data, error) {
            if (!data) {
