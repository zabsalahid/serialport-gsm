...
module.exports = function (SerialPort) {
  ...
  modem.getOwnNumber = function (callback, timeout = 10000) {
    modem.executeCommand(`AT+CNUM`, function (result) {
      callback(result)
    }, false, timeout)
  }
  ...
  modem.setOwnNumber = function (number, callback, name = 'OwnNumber', timeout = 10000) {
    ...
    modem.executeCommand(`AT+CPBS="ON"`, function (result) {
      if (result.data === 'OK') {
        //CPBW parameters, Phonebook location, phone number, type of address octet in integer format, name
        modem.executeCommand(`AT+CPBW=1,"${number}",129,"${name}"`, function (result) {
  ...
  modem.getSimInbox = function (callback, timeout = 15000) {
    modem.executeCommand((modem.modemMode == 0 ? `AT+CMGL=4` : `AT+CMGL="ALL"`), function (result) {
      callback(result)
    }, false, timeout)
  }

  modem.deleteMessage = function (messageIndex, callback, timeout = 10000) {
    modem.executeCommand(`AT+CMGD=${messageIndex}`, function (result) {
      if (callback) callback(result)
    }, false, timeout)
  }

  self.processMessage = function (newMessage, messageIndex) {
    let message = {
