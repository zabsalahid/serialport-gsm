...
module.exports = function(SerialPort) {
  ...
  modem.incomingSMSIndicationDelay = 3000;
  ...
  modem.emitDelayedNewIncomingMessageByReference = referenceNumber => {
    ...
    if (resultData) {
      ...
      self.emitNewIncomingMessage(resultData)
    }
  }
    ...
    parts.forEach(part => {
      ...
      newparts.forEach(newpart => {
        ...
        if (modem.incomingSMSIndication && newpart.startsWith('+CMTI:')) { // New Message Indicator with SIM Card ID, After Receiving Read The Message From the SIM Card
          ...
          modem.readSMSById(id, (error, result) => {
            if (!error) {
              if (result && result.data && result.data.udh) {
                let messageData = modem.incomingSMSIndicationTimeoutsByReference.get(result.data.udh.referenceNumber)
                if (messageData) {
                ...
                if (messageData.data.length !== result.data.udh.parts) {
                  messageData.emitDelay = setTimeout((referenceNumber) =>
                    self.emitDelayedNewIncomingMessageByReference(referenceNumber),
                    modem.incomingSMSIndicationDelay, result.data.udh.referenceNumber)
                  modem.incomingSMSIndicationTimeoutsByReference.set(result.data.udh.referenceNumber, messageData)
                  return
                }
                modem.incomingSMSIndicationTimeoutsByReference.delete(result.data.udh.referenceNumber)
                result = messageData
              }
              self.emitNewIncomingMessage(result)
            }
          });

          if (modem.incomingSMSIndicationTimeoutsByReference.has(id)) {
            clearTimeout(modem.incomingSMSIndicationTimeoutsByReference.get(id))
          }
          modem.incomingSMSIndicationTimeoutsByReference.set(id, setTimeout((id) => {
            modem.incomingSMSIndicationTimeoutsByReference.delete(id)

          }, modem.incomingSMSIndicationDelay, id))
